    ...
    function renderRequestStats(data) {
      ...
      if (Object.keys(s.byRole).length) {
        ...
        Object.entries(s.byRole).sort((a,b) => a[0].localeCompare(b[0])).forEach(([role, v], i) => {
          if (i > 0) out += '<div style="height:1px;background:#c7d1d7;margin:8px 0;"></div>';
          out += html`<div style="display:flex;align-items:center;gap:10px;">
            <span style="flex:1;font-size:14px;font-weight:bold;">${role}</span>
      ...
      if (data.requests.length) {
        out += '<div class="section-header" style="margin-bottom:10px;">Senaste förfrågningar</div>';
        ...
        data.requests.slice(0, 20).forEach(r => {
          ...
          out += html`<div style="background:#eef1f3;border:1.5px solid #c7d1d7;border-radius:6px;padding:10px 14px;display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
            <div style="flex:1;min-width:180px;">
              ...
              <span style="font-size:13px;color:#5b6b75;"> (${r.toRole})</span>
              ${r.isSelf ? safe('<span style="font-size:12px;background:#e8f4ea;color:#2e4a5f;border-radius:4px;padding:1px 6px;margin-left:4px;">självskattning</span>') : ''}
              <div style="font-size:12px;color:#8a97a0;margin-top:2px;">${r.formType || ''} · från ${r.fromName} · ${r.timestamp}</div>
            </div>
